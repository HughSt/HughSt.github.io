...
First we are going to subset some spatial (polygon) data. For this excersize, we are going to use the admin 2 boundaries for Burkina Faso we used in week 1. As a reminder, we can load these in from a local shapefile using the readOGR function, or we can use the handy `getData` function from the `raster` package to access GADM data.
