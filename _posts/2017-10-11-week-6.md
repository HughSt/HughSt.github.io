...
where ${\\bf s}\_i = (s\_{i,1}, s\_{i,2})$ are the longitude and latitude coordinates where each measuremnt was taken.
...
A covariance function *K* : 𝕊 × 𝕊 → ℝ maps a pair of points ${\\bf s}\_1 = (s\_{1,1}, s\_{1,2})$ and ${\\bf s}\_2 = (s\_{2,1}, s\_{2,2})$ to the real line. We can define such a function in terms of the distance between a pair of points. Let the distance between the points be given by $r = \\|{\\bf s}\_1 - {\\bf s}\_2 \\|$, the following are examples of covarinace functions:

Exponentiated Quadratic: $K({\\bf s}\_1, {\\bf s}\_2 ) = \\sigma^2 \\exp \\left( -\\frac{r^2}{\\ell^2} \\right) $

Rational Quadratic: $K({\\bf s}\_1, {\\bf s}\_2 ) = \\sigma^2 \\left( 1 + \\frac{r^2}{2\\alpha \\ell^2} \\right)^{-\\alpha} $

Matern Covariance: $K({\\bf s}\_1, {\\bf s}\_2 ) = \\sigma^2 \\frac{2^{1-\\nu}}{\\Gamma(\\nu)} \\left( \\sqrt{2 \\nu} \\frac{r}{\\ell} \\right)^{\\nu} \\mathcal{K}\_\\nu \\left( \\sqrt{2\\nu} \\frac{r}{\\ell}\\right) $
...
$\\eta\_i = \\beta\_0 + \\beta\_1 x\_i + f( {\\bf s}\_i) $, where ($f( {\\bf s}\_1), \\ldots , f( {\\bf s}\_2) $) is a multivariate Gaussian with spatial covariance *K*.
